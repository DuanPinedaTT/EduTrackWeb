// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using edutrack_academy_api.Data;

#nullable disable

namespace edutrack_academy_api.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20251123021038_SeedDataRefresh")]
    partial class SeedDataRefresh
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.22")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("edutrack_academy_api.Models.Administrador", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Direccion")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("FechaRegistro")
                        .HasColumnType("datetime2");

                    b.Property<string>("Telefono")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("UsuarioId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("UsuarioId")
                        .IsUnique();

                    b.ToTable("Administradores");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Direccion = "Calle 10 #45-21",
                            FechaRegistro = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(3936),
                            Telefono = "3001112233",
                            UsuarioId = 1
                        },
                        new
                        {
                            Id = 2,
                            Direccion = "Carrera 50 #12-44",
                            FechaRegistro = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(3941),
                            Telefono = "3002223344",
                            UsuarioId = 2
                        },
                        new
                        {
                            Id = 3,
                            Direccion = "Diagonal 80 #66-01",
                            FechaRegistro = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(3942),
                            Telefono = "3003334455",
                            UsuarioId = 3
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Asignatura", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Asignaturas");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Codigo = "MAT",
                            Nombre = "Matemáticas"
                        },
                        new
                        {
                            Id = 2,
                            Codigo = "LEN",
                            Nombre = "Lengua Castellana"
                        },
                        new
                        {
                            Id = 3,
                            Codigo = "CIE",
                            Nombre = "Ciencias Naturales"
                        },
                        new
                        {
                            Id = 4,
                            Codigo = "ING",
                            Nombre = "Inglés"
                        },
                        new
                        {
                            Id = 5,
                            Codigo = "HIS",
                            Nombre = "Historia"
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Asistencia", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("CursoAsignaturaId")
                        .HasColumnType("int");

                    b.Property<int>("Estado")
                        .HasColumnType("int");

                    b.Property<int>("EstudianteId")
                        .HasColumnType("int");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("datetime2");

                    b.Property<string>("Observacion")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CursoAsignaturaId");

                    b.HasIndex("EstudianteId");

                    b.ToTable("Asistencias");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CursoAsignaturaId = 1,
                            Estado = 1,
                            EstudianteId = 1,
                            Fecha = new DateTime(2025, 2, 3, 12, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = "Participó activamente"
                        },
                        new
                        {
                            Id = 2,
                            CursoAsignaturaId = 1,
                            Estado = 1,
                            EstudianteId = 1,
                            Fecha = new DateTime(2025, 2, 4, 12, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = ""
                        },
                        new
                        {
                            Id = 3,
                            CursoAsignaturaId = 1,
                            Estado = 3,
                            EstudianteId = 1,
                            Fecha = new DateTime(2025, 2, 5, 12, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = "Llegó 10 min tarde"
                        },
                        new
                        {
                            Id = 4,
                            CursoAsignaturaId = 1,
                            Estado = 1,
                            EstudianteId = 4,
                            Fecha = new DateTime(2025, 2, 3, 12, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = ""
                        },
                        new
                        {
                            Id = 5,
                            CursoAsignaturaId = 1,
                            Estado = 2,
                            EstudianteId = 4,
                            Fecha = new DateTime(2025, 2, 4, 12, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = "Justificada por cita médica"
                        },
                        new
                        {
                            Id = 6,
                            CursoAsignaturaId = 1,
                            Estado = 1,
                            EstudianteId = 4,
                            Fecha = new DateTime(2025, 2, 5, 12, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = ""
                        },
                        new
                        {
                            Id = 7,
                            CursoAsignaturaId = 3,
                            Estado = 1,
                            EstudianteId = 2,
                            Fecha = new DateTime(2025, 2, 3, 15, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = ""
                        },
                        new
                        {
                            Id = 8,
                            CursoAsignaturaId = 3,
                            Estado = 1,
                            EstudianteId = 2,
                            Fecha = new DateTime(2025, 2, 4, 15, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = "Dirigió laboratorio"
                        },
                        new
                        {
                            Id = 9,
                            CursoAsignaturaId = 3,
                            Estado = 1,
                            EstudianteId = 5,
                            Fecha = new DateTime(2025, 2, 3, 15, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = ""
                        },
                        new
                        {
                            Id = 10,
                            CursoAsignaturaId = 3,
                            Estado = 2,
                            EstudianteId = 5,
                            Fecha = new DateTime(2025, 2, 4, 15, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = "No entregó excusa"
                        },
                        new
                        {
                            Id = 11,
                            CursoAsignaturaId = 5,
                            Estado = 1,
                            EstudianteId = 3,
                            Fecha = new DateTime(2025, 2, 3, 17, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = "Exposición sobresaliente"
                        },
                        new
                        {
                            Id = 12,
                            CursoAsignaturaId = 5,
                            Estado = 1,
                            EstudianteId = 3,
                            Fecha = new DateTime(2025, 2, 4, 17, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = ""
                        },
                        new
                        {
                            Id = 13,
                            CursoAsignaturaId = 5,
                            Estado = 3,
                            EstudianteId = 6,
                            Fecha = new DateTime(2025, 2, 3, 17, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = "Ingreso tarde por transporte"
                        },
                        new
                        {
                            Id = 14,
                            CursoAsignaturaId = 5,
                            Estado = 1,
                            EstudianteId = 6,
                            Fecha = new DateTime(2025, 2, 4, 17, 0, 0, 0, DateTimeKind.Utc),
                            Observacion = ""
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Curso", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("AsignaturaId")
                        .HasColumnType("int");

                    b.Property<int?>("GradoId")
                        .HasColumnType("int");

                    b.Property<string>("Grupo")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("ProfesorId")
                        .HasColumnType("int");

                    b.Property<int?>("UsuarioId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("AsignaturaId");

                    b.HasIndex("GradoId");

                    b.HasIndex("ProfesorId");

                    b.HasIndex("UsuarioId");

                    b.ToTable("Cursos");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            GradoId = 1,
                            Grupo = "A",
                            Nombre = "5° Básico A",
                            ProfesorId = 1
                        },
                        new
                        {
                            Id = 2,
                            GradoId = 2,
                            Grupo = "A",
                            Nombre = "8° Básico A",
                            ProfesorId = 2
                        },
                        new
                        {
                            Id = 3,
                            GradoId = 3,
                            Grupo = "A",
                            Nombre = "10° Académico A",
                            ProfesorId = 3
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.CursoAsignatura", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("AsignaturaId")
                        .HasColumnType("int");

                    b.Property<int>("CursoId")
                        .HasColumnType("int");

                    b.Property<int?>("ProfesorId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("AsignaturaId");

                    b.HasIndex("ProfesorId");

                    b.HasIndex("CursoId", "AsignaturaId")
                        .IsUnique();

                    b.ToTable("CursoAsignaturas");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            AsignaturaId = 1,
                            CursoId = 1,
                            ProfesorId = 1
                        },
                        new
                        {
                            Id = 2,
                            AsignaturaId = 4,
                            CursoId = 1,
                            ProfesorId = 3
                        },
                        new
                        {
                            Id = 3,
                            AsignaturaId = 3,
                            CursoId = 2,
                            ProfesorId = 2
                        },
                        new
                        {
                            Id = 4,
                            AsignaturaId = 2,
                            CursoId = 2,
                            ProfesorId = 1
                        },
                        new
                        {
                            Id = 5,
                            AsignaturaId = 4,
                            CursoId = 3,
                            ProfesorId = 3
                        },
                        new
                        {
                            Id = 6,
                            AsignaturaId = 5,
                            CursoId = 3,
                            ProfesorId = 2
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Estudiante", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Acudiente")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Apellido")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Direccion")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Documento")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("FechaNacimiento")
                        .HasColumnType("datetime2");

                    b.Property<string>("Nivel")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Telefono")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("UsuarioId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("UsuarioId")
                        .IsUnique()
                        .HasFilter("[UsuarioId] IS NOT NULL");

                    b.ToTable("Estudiantes");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Acudiente = "Patricia Gómez",
                            Apellido = "Marín",
                            Direccion = "Barrio Primavera",
                            Documento = "1053891201",
                            FechaNacimiento = new DateTime(2014, 5, 17, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nivel = "5°",
                            Nombre = "Luisa",
                            Telefono = "3015558899",
                            UsuarioId = 7
                        },
                        new
                        {
                            Id = 2,
                            Acudiente = "Sandra Ruiz",
                            Apellido = "Ruiz",
                            Direccion = "Conjunto Nogales",
                            Documento = "1054782203",
                            FechaNacimiento = new DateTime(2012, 11, 9, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nivel = "8°",
                            Nombre = "Carlos",
                            Telefono = "3017776644",
                            UsuarioId = 8
                        },
                        new
                        {
                            Id = 3,
                            Acudiente = "Marcos Suárez",
                            Apellido = "Suárez",
                            Direccion = "Urbanización Cedros",
                            Documento = "1045221188",
                            FechaNacimiento = new DateTime(2010, 3, 28, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nivel = "10°",
                            Nombre = "Ana",
                            Telefono = "3027773311",
                            UsuarioId = 9
                        },
                        new
                        {
                            Id = 4,
                            Acudiente = "Carolina López",
                            Apellido = "López",
                            Direccion = "Villa del Prado",
                            Documento = "1053999981",
                            FechaNacimiento = new DateTime(2014, 8, 5, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nivel = "5°",
                            Nombre = "Mateo",
                            Telefono = "3009007766"
                        },
                        new
                        {
                            Id = 5,
                            Acudiente = "Andrea Ortiz",
                            Apellido = "Ortiz",
                            Direccion = "Bosques del Norte",
                            Documento = "1045888812",
                            FechaNacimiento = new DateTime(2012, 1, 15, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nivel = "8°",
                            Nombre = "Valentina",
                            Telefono = "3012203344"
                        },
                        new
                        {
                            Id = 6,
                            Acudiente = "Ricardo Torres",
                            Apellido = "Torres",
                            Direccion = "Altos de la Sabana",
                            Documento = "1050011223",
                            FechaNacimiento = new DateTime(2010, 9, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nivel = "10°",
                            Nombre = "Samuel",
                            Telefono = "3021144556"
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Grado", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Grupos")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Grados");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Codigo = "5BAS",
                            Grupos = "A,B",
                            Nombre = "Quinto Básico"
                        },
                        new
                        {
                            Id = 2,
                            Codigo = "8BAS",
                            Grupos = "A,B",
                            Nombre = "Octavo Básico"
                        },
                        new
                        {
                            Id = 3,
                            Codigo = "10ACA",
                            Grupos = "A",
                            Nombre = "Décimo Académico"
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Inscripcion", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("CursoId")
                        .HasColumnType("int");

                    b.Property<int>("EstudianteId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CursoId");

                    b.HasIndex("EstudianteId", "CursoId")
                        .IsUnique();

                    b.ToTable("Inscripciones");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CursoId = 1,
                            EstudianteId = 1
                        },
                        new
                        {
                            Id = 2,
                            CursoId = 1,
                            EstudianteId = 4
                        },
                        new
                        {
                            Id = 3,
                            CursoId = 2,
                            EstudianteId = 2
                        },
                        new
                        {
                            Id = 4,
                            CursoId = 2,
                            EstudianteId = 5
                        },
                        new
                        {
                            Id = 5,
                            CursoId = 3,
                            EstudianteId = 3
                        },
                        new
                        {
                            Id = 6,
                            CursoId = 3,
                            EstudianteId = 6
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Nota", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("CursoId")
                        .HasColumnType("int");

                    b.Property<int>("EstudianteId")
                        .HasColumnType("int");

                    b.Property<int>("NotaConfigId")
                        .HasColumnType("int");

                    b.Property<decimal?>("Valor")
                        .HasPrecision(4, 2)
                        .HasColumnType("decimal(4,2)");

                    b.HasKey("Id");

                    b.HasIndex("CursoId");

                    b.HasIndex("EstudianteId");

                    b.HasIndex("NotaConfigId");

                    b.ToTable("Notas");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            EstudianteId = 1,
                            NotaConfigId = 1,
                            Valor = 4.5m
                        },
                        new
                        {
                            Id = 2,
                            EstudianteId = 1,
                            NotaConfigId = 2,
                            Valor = 4.2m
                        },
                        new
                        {
                            Id = 3,
                            EstudianteId = 4,
                            NotaConfigId = 1,
                            Valor = 3.8m
                        },
                        new
                        {
                            Id = 4,
                            EstudianteId = 4,
                            NotaConfigId = 2,
                            Valor = 3.5m
                        },
                        new
                        {
                            Id = 5,
                            EstudianteId = 2,
                            NotaConfigId = 3,
                            Valor = 4.0m
                        },
                        new
                        {
                            Id = 6,
                            EstudianteId = 2,
                            NotaConfigId = 4,
                            Valor = 4.6m
                        },
                        new
                        {
                            Id = 7,
                            EstudianteId = 5,
                            NotaConfigId = 3,
                            Valor = 3.2m
                        },
                        new
                        {
                            Id = 8,
                            EstudianteId = 5,
                            NotaConfigId = 4,
                            Valor = 3.4m
                        },
                        new
                        {
                            Id = 9,
                            EstudianteId = 3,
                            NotaConfigId = 5,
                            Valor = 4.8m
                        },
                        new
                        {
                            Id = 10,
                            EstudianteId = 3,
                            NotaConfigId = 6,
                            Valor = 4.4m
                        },
                        new
                        {
                            Id = 11,
                            EstudianteId = 6,
                            NotaConfigId = 5,
                            Valor = 3.9m
                        },
                        new
                        {
                            Id = 12,
                            EstudianteId = 6,
                            NotaConfigId = 6,
                            Valor = 3.6m
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.NotaConfig", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("CursoId")
                        .HasColumnType("int");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Orden")
                        .HasColumnType("int");

                    b.Property<int>("PeriodoAcademicoId")
                        .HasColumnType("int");

                    b.Property<decimal>("Peso")
                        .HasPrecision(5, 2)
                        .HasColumnType("decimal(5,2)");

                    b.HasKey("Id");

                    b.HasIndex("CursoId");

                    b.HasIndex("PeriodoAcademicoId");

                    b.ToTable("NotaConfigs");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CursoId = 1,
                            Nombre = "Proyecto STEAM",
                            Orden = 1,
                            PeriodoAcademicoId = 1,
                            Peso = 50m
                        },
                        new
                        {
                            Id = 2,
                            CursoId = 1,
                            Nombre = "Evaluación integral",
                            Orden = 2,
                            PeriodoAcademicoId = 1,
                            Peso = 50m
                        },
                        new
                        {
                            Id = 3,
                            CursoId = 2,
                            Nombre = "Laboratorio de ciencias",
                            Orden = 1,
                            PeriodoAcademicoId = 1,
                            Peso = 40m
                        },
                        new
                        {
                            Id = 4,
                            CursoId = 2,
                            Nombre = "Examen bimestral",
                            Orden = 2,
                            PeriodoAcademicoId = 1,
                            Peso = 60m
                        },
                        new
                        {
                            Id = 5,
                            CursoId = 3,
                            Nombre = "Ensayo crítico",
                            Orden = 1,
                            PeriodoAcademicoId = 1,
                            Peso = 40m
                        },
                        new
                        {
                            Id = 6,
                            CursoId = 3,
                            Nombre = "Examen final",
                            Orden = 2,
                            PeriodoAcademicoId = 1,
                            Peso = 60m
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Notificacion", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("CursoAsignaturaId")
                        .HasColumnType("int");

                    b.Property<int?>("EstudianteId")
                        .HasColumnType("int");

                    b.Property<DateTime>("FechaEnvio")
                        .HasColumnType("datetime2");

                    b.Property<bool>("Leida")
                        .HasColumnType("bit");

                    b.Property<string>("Mensaje")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ProfesorId")
                        .HasColumnType("int");

                    b.Property<string>("Tipo")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Titulo")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CursoAsignaturaId");

                    b.HasIndex("EstudianteId");

                    b.HasIndex("ProfesorId");

                    b.ToTable("Notificaciones");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CursoAsignaturaId = 1,
                            FechaEnvio = new DateTime(2025, 2, 2, 13, 0, 0, 0, DateTimeKind.Utc),
                            Leida = false,
                            Mensaje = "Recuerden cargar la presentación antes del viernes.",
                            ProfesorId = 1,
                            Tipo = "evaluacion",
                            Titulo = "Entrega de proyecto STEAM"
                        },
                        new
                        {
                            Id = 2,
                            CursoAsignaturaId = 5,
                            EstudianteId = 3,
                            FechaEnvio = new DateTime(2025, 2, 5, 18, 0, 0, 0, DateTimeKind.Utc),
                            Leida = true,
                            Mensaje = "Excelente liderazgo en la exposición final.",
                            ProfesorId = 3,
                            Tipo = "reconocimiento",
                            Titulo = "Reconocimiento"
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Observacion", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Comentario")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CursoAsignaturaId")
                        .HasColumnType("int");

                    b.Property<int>("EstudianteId")
                        .HasColumnType("int");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("datetime2");

                    b.Property<int>("ProfesorId")
                        .HasColumnType("int");

                    b.Property<string>("Tipo")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CursoAsignaturaId");

                    b.HasIndex("EstudianteId");

                    b.HasIndex("ProfesorId");

                    b.ToTable("Observaciones");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Comentario = "Requiere refuerzo en registro de laboratorio",
                            CursoAsignaturaId = 3,
                            EstudianteId = 5,
                            Fecha = new DateTime(2025, 2, 6, 14, 30, 0, 0, DateTimeKind.Utc),
                            ProfesorId = 2,
                            Tipo = "seguimiento"
                        },
                        new
                        {
                            Id = 2,
                            Comentario = "Lideró al equipo durante el reto de robótica",
                            CursoAsignaturaId = 1,
                            EstudianteId = 1,
                            Fecha = new DateTime(2025, 2, 7, 10, 0, 0, 0, DateTimeKind.Utc),
                            ProfesorId = 1,
                            Tipo = "reconocimiento"
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.PeriodoAcademico", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<bool>("Activo")
                        .HasColumnType("bit");

                    b.Property<DateTime>("FechaFin")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("FechaInicio")
                        .HasColumnType("datetime2");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("nvarchar(120)");

                    b.Property<int>("Orden")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("PeriodosAcademicos");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Activo = true,
                            FechaFin = new DateTime(2025, 3, 31, 23, 59, 59, 0, DateTimeKind.Utc),
                            FechaInicio = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Periodo 1",
                            Orden = 1
                        },
                        new
                        {
                            Id = 2,
                            Activo = false,
                            FechaFin = new DateTime(2025, 6, 30, 23, 59, 59, 0, DateTimeKind.Utc),
                            FechaInicio = new DateTime(2025, 4, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Periodo 2",
                            Orden = 2
                        },
                        new
                        {
                            Id = 3,
                            Activo = false,
                            FechaFin = new DateTime(2025, 9, 30, 23, 59, 59, 0, DateTimeKind.Utc),
                            FechaInicio = new DateTime(2025, 7, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Periodo 3",
                            Orden = 3
                        },
                        new
                        {
                            Id = 4,
                            Activo = false,
                            FechaFin = new DateTime(2025, 12, 31, 23, 59, 59, 0, DateTimeKind.Utc),
                            FechaInicio = new DateTime(2025, 10, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Periodo 4",
                            Orden = 4
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Profesor", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Biografia")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Direccion")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Especialidad")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("FechaIngreso")
                        .HasColumnType("datetime2");

                    b.Property<string>("Telefono")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("UsuarioId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("UsuarioId")
                        .IsUnique();

                    b.ToTable("Profesores");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Biografia = "Mentora STEM con enfoque en innovación",
                            Direccion = "Av. Educativa 45",
                            Especialidad = "Matemáticas",
                            FechaIngreso = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(4160),
                            Telefono = "3009876543",
                            UsuarioId = 4
                        },
                        new
                        {
                            Id = 2,
                            Biografia = "Coordinador de laboratorios escolares",
                            Direccion = "Calle 23 #18-55",
                            Especialidad = "Ciencias Naturales",
                            FechaIngreso = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(4164),
                            Telefono = "3008765432",
                            UsuarioId = 5
                        },
                        new
                        {
                            Id = 3,
                            Biografia = "Docente bilingüe con certificación CELTA",
                            Direccion = "Transversal 90 #33-10",
                            Especialidad = "Inglés",
                            FechaIngreso = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(4165),
                            Telefono = "3007654321",
                            UsuarioId = 6
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Usuario", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Apellido")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreadoEn")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Rol")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("User")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("User")
                        .IsUnique();

                    b.ToTable("Usuarios");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Apellido = "Medina",
                            CreadoEn = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(3788),
                            Email = "laura.medina@edutrack.com",
                            Nombre = "Laura",
                            PasswordHash = "$2a$11$b6uWBSEetnNYUt0fR2N2b.C9yA443GndOniPIOy4RqVHKfU6EXylK",
                            Rol = "admin",
                            User = "admin.campus"
                        },
                        new
                        {
                            Id = 2,
                            Apellido = "Herrera",
                            CreadoEn = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(3796),
                            Email = "diego.herrera@edutrack.com",
                            Nombre = "Diego",
                            PasswordHash = "$2a$11$F40K44fW8J0gyyoWXPf4nOJRe/ZHHp6nzhAGKyolfCYW6a9xshRuu",
                            Rol = "admin",
                            User = "coordinacion"
                        },
                        new
                        {
                            Id = 3,
                            Apellido = "Roldán",
                            CreadoEn = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(3798),
                            Email = "sofia.roldan@edutrack.com",
                            Nombre = "Sofía",
                            PasswordHash = "$2a$11$4zUFBLeIJfBJjwvGzZhYOOFseetOcdFGVQyW1NUcE9oGNzxFMlMYG",
                            Rol = "admin",
                            User = "rectoria"
                        },
                        new
                        {
                            Id = 4,
                            Apellido = "Valencia",
                            CreadoEn = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(3799),
                            Email = "martina.valencia@edutrack.com",
                            Nombre = "Martina",
                            PasswordHash = "$2a$11$TJbxu6dVaNyjhBDn7t2hS.cn32ihxTNltcSk5r97MuBZliRFbBvy.",
                            Rol = "docente",
                            User = "prof.mvalencia"
                        },
                        new
                        {
                            Id = 5,
                            Apellido = "Ramírez",
                            CreadoEn = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(3801),
                            Email = "julio.ramirez@edutrack.com",
                            Nombre = "Julio",
                            PasswordHash = "$2a$11$hPC66/NNzgrMtMN6odDfwer5ztJz9jRH8qPuBg9ve9NcYQJzRgnKO",
                            Rol = "docente",
                            User = "prof.jramirez"
                        },
                        new
                        {
                            Id = 6,
                            Apellido = "Zamora",
                            CreadoEn = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(3804),
                            Email = "lucia.zamora@edutrack.com",
                            Nombre = "Lucía",
                            PasswordHash = "$2a$11$A7MyTpXoRR7Ijnwt/3dMqemAARofRpgBLsX0dk6IvLvqvqBc89lKO",
                            Rol = "docente",
                            User = "prof.zamora"
                        },
                        new
                        {
                            Id = 7,
                            Apellido = "Marín",
                            CreadoEn = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(3805),
                            Email = "luisa.marin@edutrack.com",
                            Nombre = "Luisa",
                            PasswordHash = "$2a$11$UBqkCYm1rmnMZtalVJ2EHeXCjZvcwt/Cckf731zIzIgI1fLRe/flW",
                            Rol = "estudiante",
                            User = "est.luisa"
                        },
                        new
                        {
                            Id = 8,
                            Apellido = "Ruiz",
                            CreadoEn = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(3806),
                            Email = "carlos.ruiz@edutrack.com",
                            Nombre = "Carlos",
                            PasswordHash = "$2a$11$rRq2ylS2LrM0m2A51QQOdOiReLqOUyiGePHucAkaco4eqivUi/QYi",
                            Rol = "estudiante",
                            User = "est.carlos"
                        },
                        new
                        {
                            Id = 9,
                            Apellido = "Suárez",
                            CreadoEn = new DateTime(2025, 11, 23, 2, 10, 37, 978, DateTimeKind.Utc).AddTicks(3808),
                            Email = "ana.suarez@edutrack.com",
                            Nombre = "Ana",
                            PasswordHash = "$2a$11$.9aZfR3Pg1.B8vLwjvgxsuShtAOMxegHN9gfek0Jj.J0zUe94y7w6",
                            Rol = "estudiante",
                            User = "est.ana"
                        });
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Administrador", b =>
                {
                    b.HasOne("edutrack_academy_api.Models.Usuario", "Usuario")
                        .WithOne("AdministradorPerfil")
                        .HasForeignKey("edutrack_academy_api.Models.Administrador", "UsuarioId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Asistencia", b =>
                {
                    b.HasOne("edutrack_academy_api.Models.CursoAsignatura", "CursoAsignatura")
                        .WithMany()
                        .HasForeignKey("CursoAsignaturaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("edutrack_academy_api.Models.Estudiante", "Estudiante")
                        .WithMany("Asistencias")
                        .HasForeignKey("EstudianteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CursoAsignatura");

                    b.Navigation("Estudiante");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Curso", b =>
                {
                    b.HasOne("edutrack_academy_api.Models.Asignatura", null)
                        .WithMany("Cursos")
                        .HasForeignKey("AsignaturaId");

                    b.HasOne("edutrack_academy_api.Models.Grado", "Grado")
                        .WithMany("Cursos")
                        .HasForeignKey("GradoId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("edutrack_academy_api.Models.Profesor", "Profesor")
                        .WithMany("CursosTitular")
                        .HasForeignKey("ProfesorId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("edutrack_academy_api.Models.Usuario", null)
                        .WithMany("CursosAsignados")
                        .HasForeignKey("UsuarioId");

                    b.Navigation("Grado");

                    b.Navigation("Profesor");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.CursoAsignatura", b =>
                {
                    b.HasOne("edutrack_academy_api.Models.Asignatura", "Asignatura")
                        .WithMany()
                        .HasForeignKey("AsignaturaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("edutrack_academy_api.Models.Curso", "Curso")
                        .WithMany("CursoAsignaturas")
                        .HasForeignKey("CursoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("edutrack_academy_api.Models.Profesor", "Profesor")
                        .WithMany("Asignaturas")
                        .HasForeignKey("ProfesorId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Asignatura");

                    b.Navigation("Curso");

                    b.Navigation("Profesor");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Estudiante", b =>
                {
                    b.HasOne("edutrack_academy_api.Models.Usuario", "Usuario")
                        .WithOne("EstudiantePerfil")
                        .HasForeignKey("edutrack_academy_api.Models.Estudiante", "UsuarioId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Inscripcion", b =>
                {
                    b.HasOne("edutrack_academy_api.Models.Curso", "Curso")
                        .WithMany("Inscripciones")
                        .HasForeignKey("CursoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("edutrack_academy_api.Models.Estudiante", "Estudiante")
                        .WithMany("Inscripciones")
                        .HasForeignKey("EstudianteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Curso");

                    b.Navigation("Estudiante");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Nota", b =>
                {
                    b.HasOne("edutrack_academy_api.Models.Curso", null)
                        .WithMany("Notas")
                        .HasForeignKey("CursoId");

                    b.HasOne("edutrack_academy_api.Models.Estudiante", "Estudiante")
                        .WithMany("Notas")
                        .HasForeignKey("EstudianteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("edutrack_academy_api.Models.NotaConfig", "NotaConfig")
                        .WithMany()
                        .HasForeignKey("NotaConfigId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Estudiante");

                    b.Navigation("NotaConfig");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.NotaConfig", b =>
                {
                    b.HasOne("edutrack_academy_api.Models.Curso", "Curso")
                        .WithMany()
                        .HasForeignKey("CursoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("edutrack_academy_api.Models.PeriodoAcademico", "PeriodoAcademico")
                        .WithMany("ConfiguracionesNotas")
                        .HasForeignKey("PeriodoAcademicoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Curso");

                    b.Navigation("PeriodoAcademico");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Notificacion", b =>
                {
                    b.HasOne("edutrack_academy_api.Models.CursoAsignatura", "CursoAsignatura")
                        .WithMany()
                        .HasForeignKey("CursoAsignaturaId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("edutrack_academy_api.Models.Estudiante", "Estudiante")
                        .WithMany("Notificaciones")
                        .HasForeignKey("EstudianteId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("edutrack_academy_api.Models.Profesor", "Profesor")
                        .WithMany()
                        .HasForeignKey("ProfesorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CursoAsignatura");

                    b.Navigation("Estudiante");

                    b.Navigation("Profesor");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Observacion", b =>
                {
                    b.HasOne("edutrack_academy_api.Models.CursoAsignatura", "CursoAsignatura")
                        .WithMany()
                        .HasForeignKey("CursoAsignaturaId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("edutrack_academy_api.Models.Estudiante", "Estudiante")
                        .WithMany("Observaciones")
                        .HasForeignKey("EstudianteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("edutrack_academy_api.Models.Profesor", "Profesor")
                        .WithMany()
                        .HasForeignKey("ProfesorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CursoAsignatura");

                    b.Navigation("Estudiante");

                    b.Navigation("Profesor");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Profesor", b =>
                {
                    b.HasOne("edutrack_academy_api.Models.Usuario", "Usuario")
                        .WithOne("ProfesorPerfil")
                        .HasForeignKey("edutrack_academy_api.Models.Profesor", "UsuarioId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Asignatura", b =>
                {
                    b.Navigation("Cursos");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Curso", b =>
                {
                    b.Navigation("CursoAsignaturas");

                    b.Navigation("Inscripciones");

                    b.Navigation("Notas");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Estudiante", b =>
                {
                    b.Navigation("Asistencias");

                    b.Navigation("Inscripciones");

                    b.Navigation("Notas");

                    b.Navigation("Notificaciones");

                    b.Navigation("Observaciones");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Grado", b =>
                {
                    b.Navigation("Cursos");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.PeriodoAcademico", b =>
                {
                    b.Navigation("ConfiguracionesNotas");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Profesor", b =>
                {
                    b.Navigation("Asignaturas");

                    b.Navigation("CursosTitular");
                });

            modelBuilder.Entity("edutrack_academy_api.Models.Usuario", b =>
                {
                    b.Navigation("AdministradorPerfil");

                    b.Navigation("CursosAsignados");

                    b.Navigation("EstudiantePerfil");

                    b.Navigation("ProfesorPerfil");
                });
#pragma warning restore 612, 618
        }
    }
}
